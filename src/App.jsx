...
export default function App() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const res = await fetch("https://mytodo-backend-zdna.onrender.com/tasks");
  ...
  const toggleTask = async (id, completed) => {
    try {
      const res = await fetch(`https://mytodo-backend-zdna.onrender.com/tasks/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const deleteTask = async (id) => {
    await fetch(`https://mytodo-backend-zdna.onrender.com/tasks/${id}`, { method: "DELETE" });
